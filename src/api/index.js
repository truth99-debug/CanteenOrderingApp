...
export const validateUserJWTToken = async (username , password) => {
  try {
...
export const registration = async (firstName, lastName, userEmail, password) => {
  try {
...
// add new product
export const addNewProduct = async (category) => {
  try {
    const res = await axios.post(`${baseURL}/mealitems/getByCategory`, {category});
...
export const getAllProducts = async () => {
  try {
    const res = await axios.get(`${baseURL}/mealitems/getMealItems`);
    return res.data.data;
  } catch (err) {
    return null;
  }
